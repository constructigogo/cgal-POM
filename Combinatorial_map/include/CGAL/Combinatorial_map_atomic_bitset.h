...
public:
   ...
   */
    ...
    /**
     * Flips the bit at index idx
     */
    bool flip(size_t idx, std::memory_order order = std::memory_order_seq_cst);
...
private:
    ...
    static constexpr BlockType kZero = 0;
...
inline bool AtomicBitset<N>::flip(size_t idx, std::memory_order order)
{
    ...
    if(current) {
        mask = ~mask;
        return data_[blockIndex(idx)] &= mask;
    } else {
         return data_[blockIndex(idx)] |= mask;
    }
}
